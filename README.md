...
elf2x68k + newlib環境向けの引数クォート対応コマンドライン解析ライブラリです。

XC2.1のスタートアップは二重引用符`"`や一重引用符`'`を使った引数のクォートに対応しており、空白文字の入ったファイル名やメッセージを渡すことができるのですが、newlibのスタートアップはそのようなサポートがありません。
